...
def liveness_analysis(module):
    ...
    for func in module['functions']:
        ...
        live_outs = {node: set() for node in blocks}
        live_ins = {node: set() for node in blocks}
        ...
        while len(work_list) != 0:
            ...
            live_outs[block].union(live_ins[n] for n in successor_map[block])
            # Update the live-ins set using the transfer operator
            current_live_ins = get_uses_with_no_defs(block).union(live_outs[block] - get_defs(block))

            if current_live_ins != live_ins[block]:
                live_ins[block] = current_live_ins
                work_list += predecessor_map[block]

        for block in blocks:
            print('{}:'.format(block))
            print('  in: ', fmt(live_ins[block]))
            print('  out:', fmt(live_outs[block]))
