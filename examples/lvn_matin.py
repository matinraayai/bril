...
class ValueNumberTable:
    ...
    def get_canonical_of_value(self, val):
        # print("canonical: ", val, self.value_to_canonical_variable_map[val])
    ...
    def get_or_create_use_value(self, variable):
        ...
        # print(self.variable_to_value_map)
        if var_name not in self.variable_to_value_map:
            # print("creating variable", var_name)
            self.variable_to_value_map[var_name] = self.next_value
            self.value_to_canonical_variable_map[self.next_value] = var_name
            self.next_value += 1
        # print("get or create use res: ", variable, var_name, self.variable_to_value_map[var_name])
        # print(self.value_to_canonical_variable_map[self.next_value - 1])
    ...
    def get_or_create_def_value(self, instr):
        if "dest" in instr:
            ...
            # print("creating def variable", dest_var_name, self.next_value)
            ...
            if expr not in self.expr_to_value_map:
                ...
                self.next_value += 1
            # print("Create def res:", dest_variable, expr, dest_var_name, self.expr_to_value_map[expr])
            return self.expr_to_value_map[expr]
        else:
            return None
    ...
    def get_expr_of_instr(self, inst):
        ...
        if "args" in inst:
            for arg in inst["args"]:
                expr.append(self.get_or_create_use_value(arg))
        return tuple(expr)
